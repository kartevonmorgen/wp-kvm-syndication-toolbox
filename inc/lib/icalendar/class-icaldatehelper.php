...
class ICalDateHelper 
{
	...
	function DayInMonth($month, $year) 
  {
	 ...
	 * @return bool
	 */
	function isToday($date, $tzid = "UTC") 
  {
	 ...
	 * @return bool
	 */
	function isBeforeToday($date, $tzid = "UTC")
  {
		$dtz = new DateTimeZone($tzid);
		$dt = new DateTime("now", $dtz);
		$now = time() + $dtz->getOffset($dt);
	 ...
	 * @return bool
	 */
	function isAfterToday($date, $tzid = "UTC")
  {
		$dtz = new DateTimeZone($tzid);
		$dt = new DateTime("now", $dtz);
		$now = time() + $dtz->getOffset($dt);
	 ...
	 * @return bool
	 */
	function isTomorrow($date, $tzid = "UTC") 
  {
		$dtz = new DateTimeZone($tzid);
		$dt = new DateTime("now", $dtz);
		$now = time() + $dtz->getOffset($dt);
	 ...
	 * @return bool
	 */
	function isFuture($date, $tzid = "UTC")
  {
		$dtz = new DateTimeZone($tzid);
		$dt = new DateTime("now", $dtz);
		$now = time() + $dtz->getOffset($dt);
	...
	function fromiCaltoUnixDateTime($datetime, $timezone) 
  {
	 ...
	 * @return string
	 */
	function fromUnixDateTimetoiCal($datetime)
  {
		date_default_timezone_set('GMT');
	 ...
	 * return int 
	 */
	function iCalDurationtoSeconds($duration) 
  {
		$secs = 0;
		if($duration[0] == "P") {
	 ...
	 * @return bool
	 */
	function inDay($daystart, $begin, $end)
	{
	 ...
	 */
	function addDate($date, $hour, $min, $sec, $month, $day, $year, $tzid = "UTC") {
	 ...
	 * @return int Unix timestamp
	 */
	function getDateFromDay($date, $week, $wday,$tzid="UTC") {
	 ...
	 */
	function toLocalDateTime($sqldate, $tzid = "UTC" ){
		try
		{
	 ...
	 */
	function getAbsDate($date,$rdate = ""){
	 ...
	 */
	function toiCalDateTime($datetime = null){
		date_default_timezone_set('UTC');
		if($datetime == null)
			$datetime = time();
		return gmdate("Ymd\THis",$datetime);
	 ...
	 * @return string iCal date-time string
	 */
	function toiCalDate($datetime = null){
		date_default_timezone_set('UTC');
		if($datetime == null)
			$datetime = time();
